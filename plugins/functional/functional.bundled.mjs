...
function generate$1(node, options) {
  /*
...
var astring = /*#__PURE__*/Object.freeze({
  __proto__: null,
...
const generate = (generate$1 ?? undefined);
/** MISC **/
function replaceObjectProperties(obj, newObject) {
    [
...
function createBlockStatementNode(params) {
    return {
        ...params,
        type: 'BlockStatement',
    };
}
function createReturnStatementNode(params) {
    return {
        ...params,
        type: 'ReturnStatement',
    };
}
function createIdentifierNode(params) {
    return {
        ...params,
        type: 'Identifier',
    };
}
function createArrowFunctionExpressionNode(params) {
    return {
        ...params,
        type: 'ArrowFunctionExpression',
    };
}
function createFunctionDeclarationNode(params) {
    return {
        ...params,
...
/** PIPE NODE **/
function isPipeNode(node, pipeFunctionName) {
...
function extractPipeNodeElements(node, pipeFunctionName) {
    if (isPipeNode(node, pipeFunctionName)) {
        if (node.arguments.length === 1) {
            const arg0 = node.arguments[0];
            if (isArrayExpressionNode(arg0)) {
                return extractPipeElements(arg0.elements);
            }
            else {
...
function unrollPipeNodeElements(pipes, ecmaVersion) {
...
function handlePipeNode(node, options) {
...
/** PIPE NOW NODE **/
function isPipeNowNode(node, pipeNowFunctionName) {
    return isCallExpressionNode(node)
        && isIdentifierNode(node.callee)
        && pipeNowFunctionName.has(node.callee.name);
}
function extractPipeNowNodeElements(node, pipeNowFunctionName, pipeFunctionName) {
    if (isPipeNowNode(node, pipeNowFunctionName)) {
        if (node.arguments.length === 2) {
            ...
            if (isArrayExpressionNode(arg1)) {
                return {
                    value: arg0,
                    pipes: extractPipeElements(arg1.elements),
                };
            }
            else {
                throw new Error(`Expected array`);
...
function unrollPipeNowNodeElements(elements) {
    return unrollPipes([
        elements.value,
        ...elements.pipes,
    ]);
}
function handlePipeNowNode(node, options) {
...
function optimizeAST(ast, options) {
    full(ast, (node) => {
...
function optimizeFunctional(code, options) {
    let ast;
    try {
...
export default optimizeFunctionalPlugin;
