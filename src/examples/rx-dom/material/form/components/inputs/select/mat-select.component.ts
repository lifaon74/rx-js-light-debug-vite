import {
  compileReactiveCSSAsComponentStyle, compileReactiveHTMLAsGenericComponentTemplate, Component, HTMLElementConstructor,
  IReactiveClassListValue, OnCreate, querySelectorOrThrow,
} from '@lifaon/rx-dom';
// @ts-ignore
...
import { function$$, let$$, letU$$, map$$, mergeMapS$$, share$$ } from '@lifaon/rx-js-light-shortcuts';
import { INPUT_VALUE_MODIFIER } from '../../../modifiers/input-value.modifier';
import {
  freeze, IEmitFunction, IMulticastReplayLastSource, ISubscribeFunction, IUnsubscribeFunction, merge,
  mergeUnsubscribeFunctions, single
...
interface IData {
  ...
  readonly $focused: IEmitFunction<boolean>;
  readonly $onKeyDown: IEmitFunction<KeyboardEvent>;
  readonly fieldContainerClasses$: ISubscribeFunction<IReactiveClassListValue>;

  readonly placeholder$: ISubscribeFunction<string>;
  readonly selectValue$: ISubscribeFunction<string>;

  readonly $filterInputValue$: IMulticastReplayLastSource<string>;
  readonly $onClickClearIcon: IEmitFunction<MouseEvent>;
