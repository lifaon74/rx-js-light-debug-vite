...
import { HTMLElementConstructor, IParentNode, nodeAppendChild, OnInit, removeNodeChildren } from '../../../../../../../../rx-dom/dist';
import { locateRouterOutletElement, normalizeRouterOutletSelector } from '../../../router/router-outlet/rx-router-outlet';
import {
...
export async function injectRXRoute(
  ...
  for (let i = 0, l = routes.length; i < l; i++) {
    ...
    if (extra.canActivate !== void 0) {
      const canActivateResult: ICanActivateFunctionReturn = await extra.canActivate();
      if (canActivateResult !== void 0) {
        // TODO improve
        NAVIGATION.navigate(new URL(canActivateResult.toString(), document.baseURI));
        break;
      }
    }
