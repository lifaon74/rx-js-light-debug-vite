...
import { convertRoutePathToRegExp } from './functions/convert-route-path-to-reg-exp';
...
import {
  ...
  IMulticastReplayLastSource, IReadonlyMulticastReplayLastSource, IObservable, sourceToReadonlySource, noop,
...
export type IRouteChildren = readonly IRouteChild[];
...
export const DEFAULT_LOCATE_ROUTER_OUTLET: ILocateRouterOutletFunction = (parentElement: Element): Promise<IRouterOutletElement> => {
  return locateRouterOutletElement(`[${ROUTER_OUTLET_ATTRIBUTE_NAME}]`, parentElement, new AbortController().signal); // TODO signal
};


// NAVIGATE TO

export type INavigateTo = URL | string;
...
export function createRouteResolver(
  ...
  const path: string = normalizeRoutePath(_path);
  ...
  return async (
    ...
    const match: RegExpExecArray | null = regExp.exec(pathToResolve);
...
export function createRoutesResolver(
  routes: readonly IRouteResolveFunction[],
): IRouteResolveFunction {
  return (
